...
void CHexControl::DoPaint(CDCHandle dc, RECT& rect) {
	...
	int addrLenth = m_AddressDigits;
	const std::wstring addrFormat = L"%0" + std::to_wstring(addrLenth) + L"X";
	int x = (addrLenth + 1) * m_CharWidth;
	...
	for (int y = 0;; y++) {
		...
		int jcount = std::min<int>(m_BytesPerLine / m_DataSize, count);
		for (int j = 0; j < jcount; j++) {
			...
			dc.SetBkColor(selected ? m_Colors.SeelectionBackground : back);
...
LRESULT CHexControl::OnKeyDown(UINT, WPARAM wParam, LPARAM, BOOL&) {
	...
	if (shift && m_CaretOffset != current) {
		if (alt) {
			...
			m_Selection.SetSimple(std::min(m_CaretOffset, m_Selection.GetAnchor()),
				abs(m_CaretOffset - m_Selection.GetAnchor()));
		}
		redraw = true;
	}
...
void CHexControl::DrawNumber(CDCHandle dc, int64_t offset, uint64_t value, int digitsChanged) {
	...
	if (digitsChanged < m_DataSize * 2)
		dc.TextOut(pos.x, pos.y, temp, m_DataSize * 2);
	dc.SetTextColor(RGB(255, 0, 0));
	dc.TextOut(pos.x + m_CharWidth * (m_DataSize * 2 - digitsChanged), pos.y,
		temp.Right(digitsChanged), digitsChanged);
	ShowCaret();
}
...
LRESULT CHexControl::OnMouseWheel(UINT, WPARAM wParam, LPARAM, BOOL&) {
	...
	auto keys = GET_KEYSTATE_LPARAM(wParam);
	if (keys == 0) {
		if (delta > 0)

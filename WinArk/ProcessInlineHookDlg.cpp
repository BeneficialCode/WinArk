...
LRESULT CInlineHookDlg::OnClose(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/) {
	EndDialog(0);
	return TRUE;
}
...
LRESULT CInlineHookDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/) {
	...
	RECT rect;
	GetClientRect(&rect);
	m_ProcInlineHookTable->Create(m_hWnd, rect, nullptr, WS_CHILD | WS_VSCROLL | WS_HSCROLL | WS_BORDER | WS_EX_LAYERED);
	...
	::MoveWindow(m_ProcInlineHookTable->m_hWnd, iHorizontalUnit, iVerticalUnit, width, height, false);
	m_ProcInlineHookTable->ShowWindow(SW_SHOW);
	return TRUE;
}

LRESULT CInlineHookDlg::OnSize(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/) {
	RECT rect;
	GetClientRect(&rect);
	int iHorizontalUnit = LOWORD(GetDialogBaseUnits());
	int iVerticalUnit = HIWORD(GetDialogBaseUnits());
...
void CInlineHookDlg::OnGetMinMaxInfo(LPMINMAXINFO lpMMI) {

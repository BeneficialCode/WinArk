	...
	if (!_flagTriggers) {
		...
		if (svc) {
			...
			for (auto& trigger : triggers) {
				auto desc = CDriverTable::TriggerToText(trigger);
				if (text.Find(desc) < 0) {
					if (!text.IsEmpty())
						text += L", ";
					text += desc;
...
const CString& ServiceInfoEx::GetDependencies() const {
	if (!_flagDependencies) {
		...
		if (GetConfiguration())
			_dependencies = CDriverTable::DependenciesToString(GetConfiguration()->Dependencies);
	}
...
WinSys::ServiceSidType ServiceInfoEx::GetSidType() const {
	auto svc = Service::Open(_name, ServiceAccessMask::QueryConfig);
	ServiceSidType type = ServiceSidType::Unknow;
	if (svc)
		type = svc->GetSidType();
	return type;
}
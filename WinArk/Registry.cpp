...
bool Registry::IsHiveKey(const CString& path) {
	...
	return std::find_if(hives.begin(), hives.end(), [&](auto& hive) {
		return _wcsicmp(hive.Key.c_str(), stdReg ? StdRegPathToRealPath(path) : path) == 0;
		}) != hives.end();
}
...
CString Registry::GetDataAsString(RegistryKey& key, const RegistryItem& item) {
	...
	ULONG size = std::min<ULONG>(realsize, 512UL) / sizeof(WCHAR);
	LSTATUS status;
	CString text;
	DWORD type;
...
bool Registry::IsKeyLink(HKEY hKey, PCWSTR path, CString& link) {
	HKEY hLinkKey;
	auto error = ::RegOpenKeyEx(hKey, path, REG_OPTION_OPEN_LINK, KEY_READ, &hLinkKey);
	if (ERROR_SUCCESS == error) {
		DWORD type = 0;
...
std::vector<HandleInfo> Registry::EnumKeyHandles(bool hideInaccessible) {
	...
	do
	{
		...
		auto status = NtQuerySystemInformation(static_cast<SYSTEM_INFORMATION_CLASS>(ExtendedHandleInformation),
			buffer, size, nullptr);
		if (status == 0)
			break;
